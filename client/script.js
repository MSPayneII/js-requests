...
const buttonChange = () => {
  sayHelloButton.style.backgroundColor = "#000";
  sayHelloButton.style.color = "#fff";
};
...
const changeButtonBack = () => {
  ...
  sayHelloButton.style.color = "#000";
};
...
const ohMy = () => {
  axios.get("http://localhost:3000/animals").then((response) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      newParagraph.textContent = data[i];
...
const repeatMyParam = () => {
  axios.get("http://localhost:3000/repeat/tacos").then((response) => {
    // console.log(response.data);

    let pText = document.getElementById("repeat-text");
    pText.textContent = response.data;
...
const queryGetRequest = () => {
  axios
    .get("http://localhost:3000/query-test/?query1=test1&?query2=test2")
    .then((response) => {
      console.log(response.data);
    });
};
...
document
  ...
  .addEventListener("click", queryGetRequest);
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        - I get a console log saying "You have an empty query!"

    2: Send more than 1 query on the URL -- what happened? 
        - I received an object with key value pairs. 
*/
...
const createFood = (event) => {
  ...
  axios.post("http://localhost:3000/food", body).then((response) => {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      newFood.textContent = data[i];
      document.querySelector("body").append(newFood);
    }
  });
  foodInput.value = "";
};

let submitButton = document.getElementById("add-food");

submitButton.addEventListener("click", createFood);
